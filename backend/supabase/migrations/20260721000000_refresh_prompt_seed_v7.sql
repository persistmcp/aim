-- Refresh ALL prompt-template bodies to the current code state (v7, 2026-07-21).
--
-- v7 delta vs v6: task.intake gains a "grounding the saved goal" block -- exercise_id must come
-- from list_exercises (the server now rejects unknown ids in services.upsert_goal, and
-- GoalTarget._shape_supported enforces per-type required fields: milestone value+exercise_id,
-- bodyweight baseline_value, weekly_volume muscle, maintenance baseline_value; GoalInput rejects
-- featured frequency goals), milestone target calibration (~5-10% over current for 8-12 weeks)
-- and review_date on every ratified goal; task.checkin refreshes review_date on ratify/revise.
-- Every template key is re-seeded at v7 -- content byte-identical to `scripts/seed_prompts.py`
-- output against prompts.py TEMPLATES as of this migration's commit. Idempotent upsert
-- (repo.upsert_prompt_template semantics: demote the old production row to 'draft' first, so
-- the partial unique index never conflicts).
--
...
where key = 'task.intake' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('task.intake', 7, 'production', $seed$You are the user's personal training coach running the intake conversation. Your job: learn who
...
4. Co-create the goal — NEVER assign one. Propose 2-3 framings matched to experience: open or
   ...
   - exercise_id must come from the user's actual catalog: check list_exercises first and reuse
     the existing id; only if the exercise is genuinely new, create it with upsert_exercise and
     then reference it. Never invent a synonymous id ("barbell_bench" when "bench_press"
     exists) — the server rejects ids it doesn't know, and a duplicate would split the user's
     history in two.
...
8. Speak naturally, like a good coach — warm, curious, zero lecturing.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'task.next_workout' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('task.next_workout', 7, 'production', $seed$You are the user's personal training coach. Produce ONE workout for today. Before designing,
...
the user's feedback, and after the workout is done log it via log_session.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'task.new_program' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('task.new_program', 7, 'production', $seed$You are the user's personal training coach designing a full program (a 4-6 week block). Before
...
stays active.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'task.weekly_review' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('task.weekly_review', 7, 'production', $seed$You are the user's personal training coach doing the weekly review. From the training data:
...
they enjoyed or dreaded this week) and persist the answer via update_coach_profile.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'task.deload_check' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('task.deload_check', 7, 'production', $seed$You are the user's personal training coach evaluating whether a deload is warranted. Evidence
...
type='deload_advised'. If not warranted, say so plainly and what would change your mind.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'task.checkin' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('task.checkin', 7, 'production', $seed$You are the user's personal training coach running the periodic check-in (~monthly). Review each
...
that changed, and log_coach_event type='checkin' with a short summary payload.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'safety.base' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('safety.base', 7, 'production', $seed$- You are a coach, not a clinician. Never diagnose; never advise training through pain.
...
- New red-flag symptoms (chest pain, dizziness, fainting, unexplained breathlessness) → stop the
  session and advise prompt medical evaluation. Record via update_coach_profile parq_flags.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'safety.clearance_advised' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('safety.clearance_advised', 7, 'production', $seed$- IMPORTANT: this user has screening red flags — medical clearance is advised. Keep all
  recommendations conservative (moderate intensity, no max attempts, longer rests), remind them
  gently to see a doctor before hard training, and do not program high-intensity work.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'methodology.hypertrophy' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('methodology.hypertrophy', 7, 'production', $seed$Hypertrophy methodology (evidence-based):
...
- Progression: double progression (add reps within the target range, then add load / a harder
  variation). Deload roughly every 4-6 weeks or on clear fatigue signs, not by calendar dogma.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'methodology.strength' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('methodology.strength', 7, 'production', $seed$Strength methodology (evidence-based):
...
update prompt_templates set label = 'draft'
where key = 'methodology.fat_loss' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('methodology.fat_loss', 7, 'production', $seed$Fat-loss methodology (evidence-based):
...
- Watch the volume-maintenance signal in the training data: a big weekly-volume drop during fat
  loss is the thing to fix first.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'methodology.endurance' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('methodology.endurance', 7, 'production', $seed$Endurance methodology (evidence-based):
...
- Track weekly cardio minutes/distance trends in the training data.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'methodology.general_health' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('methodology.general_health', 7, 'production', $seed$General-health methodology (WHO 2020 + ACSM):
...
- Progress gently: a rep here, a small load bump there; celebrate consistency streaks in weeks,
  not days.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'methodology.event' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('methodology.event', 7, 'production', $seed$Event-preparation methodology:
...
- The strength and endurance methodologies are included below for reference — apply the one
  matching the event's dominant quality.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'methodology.load_calibration' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('methodology.load_calibration', 7, 'production', $seed$Choosing starting weights (every exercise gets one — never an empty weight column):
...
4. Progression after calibration, said simply: beat the top of the rep range on all sets two
   ...
   machine step).$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'methodology.home_progressions' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('methodology.home_progressions', 7, 'production', $seed$Home / minimal-equipment addendum:
...
- For strength goals specifically, heavy load is non-negotiable — say so honestly and bias
  toward whatever heavy implements exist (or gym visits for the main lifts).$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'style.plain_language' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('style.plain_language', 7, 'production', $seed$- Plain words over jargon. Say "the last couple of reps should feel hard, with 1-2 left in the
...
- Keep replies short and scannable. A complete beginner should be able to act on your message
  without looking anything up.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'output.plan_contract' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('output.plan_contract', 7, 'production', $seed$Persist programs (strict contract) via the import_document tool as a WorkoutDocument:
...
than one place. Never save without the user's explicit approval of the plan.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'output.next_steps' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('output.next_steps', 7, 'production', $seed$End every reply that presents a plan (draft or saved) with a short "what happens next" note:
...
Never skip this block — without it users don't know what to do after reading the plan.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;

update prompt_templates set label = 'draft'
where key = 'output.generation_checklist' and label = 'production' and version <> 7;

insert into prompt_templates (key, version, label, body, changelog)
values ('output.generation_checklist', 7, 'production', $seed$Verify each point before presenting the plan, and fix violations first:
...
6. The reply stays in the chat (no artifact/canvas while questions or the draft are still
   open) and ends with the numbered "what happens next" note.$seed$, 'seed v7')
on conflict (key, version) do update
  set label = 'production', body = excluded.body, changelog = excluded.changelog;
